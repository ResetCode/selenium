...
public class UsGetDataFromUrlHandler {

	public static List<UsData> handle(String driverPath,Integer num) {
		...
		try {
			...
			for(int i = 0; i < num; i++) {
				...
				data.setPrimaryPhone(primaryPhone);
				streeAddress = streeAddress.replaceFirst(" ", "");
				data.setStreetAddress(streeAddress);
				data.setCity(city);
